...
export default function Chat() {
  ...
  return (
    <div className="bg-customGray p-6">
      <h1 className="text-[#555657] font-semibold text-2xl leading-[48px] mb-5">CHAT</h1>
      <div className="flex justify-between gap-6">
        {/* Left side chat list */}
        <section className="w-1/3 bg-white p-4 border rounded">
            ...
            {chats.map((chat) => (
              <li
                ...
                className={`p-2 cursor-pointer mb-3 flex items-center gap-4 ${
                  activeChat?.id === chat.id ? "bg-green-500 text-white" : "bg-gray-100"
                } rounded`}
        ...
        {/* Right side chat box */}
        <section className="w-2/3 bg-white p-4 border rounded">
          {activeChat ? (
                ...
                {activeChat.messages.map((message, index) => (
                  <p
                    ...
                    className={`mb-2 p-2 rounded max-w-[70%] ${
                      message.isUser ? "ml-auto bg-blue-500 text-white" : "mr-auto bg-gray-200 text-black"
                    }`}
                  >
                    {message.text}
                  </p>
